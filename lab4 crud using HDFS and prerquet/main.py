...
class HDFS:
    """HDFS - Simple (Hadoop-like) Distributed File System for CRUD operations"""
    
    def __init__(self, storage_path: str = "./hdfs_storage"):
        self.storage_path = Path(storage_path)
        self.storage_path.mkdir(parents=True, exist_ok=True)
        self.files = {}  # file_id -> file_info
        
    def _generate_hdfs_id(self):
        """Generate HDFS-compatible file ID"""
        return f"hdfs_{uuid.uuid4().hex[:16]}"
    ...
    def create(self, file_path: str, metadata: dict = None) -> str:
        ...
        try:
            ...
            # Generate HDFS file ID
            file_id = self._generate_hdfs_id()
...
print("[SUCCESS] HDFS initialized successfully!")
# ------------------------------
# Parquet CRUD helpers
# ------------------------------
PARQUET_PATH = os.getenv("PARQUET_PATH", "./hdfs_storage/data.parquet")

def parquet_create(data: dict) -> None:
    df = pd.DataFrame(data)
    df.to_parquet(PARQUET_PATH, index=False)
    print(f"[PARQUET] Created file at {PARQUET_PATH} with {len(df)} rows")

def parquet_read() -> pd.DataFrame | None:
    if not Path(PARQUET_PATH).exists():
        print("[PARQUET] No parquet file found")
        return None
    df = pd.read_parquet(PARQUET_PATH)
    print(f"[PARQUET] Read {len(df)} rows")
    return df

def parquet_update(data: dict) -> None:
    # simple overwrite semantics
    parquet_create(data)

def parquet_delete() -> None:
    p = Path(PARQUET_PATH)
    if p.exists():
        p.unlink()
        print(f"[PARQUET] Deleted {PARQUET_PATH}")
    else:
        print("[PARQUET] Nothing to delete")
...
def create_file(file_path, file_name=None, description=None):
    ...
    try:
        ...
        # Upload to HDFS
        file_id = hdfs.create(str(file_path), file_metadata)
        
        if file_id:
            print(f"[SUCCESS] File created with HDFS ID: {file_id}")
            print(f"[INFO] File: {file_metadata['file_name']}")
            return file_id
        else:
            print("[ERROR] Failed to create file in HDFS.")
            return None
...
def get_all_files():
    """READ: Get all files from HDFS"""
    try:
        files = hdfs.read_all()
        print("[INFO] All Files in HDFS:")
        print("-" * 50)
        
        if not files:
            print("[INFO] No files found in HDFS.")
            return []
        
        for file_info in files:
            metadata = file_info.get("metadata", {})
            print(f"HDFS ID: {file_info['file_id']}")
...
def get_file_by_id(file_id):
    """READ: Get specific file by HDFS ID"""
    try:
        file_info = hdfs.read_by_id(file_id)
        if file_info:
            metadata = file_info.get("metadata", {})
            print("[SEARCH] File found in HDFS:")
            print("-" * 40)
            print(f"HDFS ID: {file_info['file_id']}")
            print(f"Name: {metadata.get('file_name', 'Unknown')}")
            print(f"Size: {file_info['size_bytes']:,} bytes")
            ...
            print("[ERROR] No file found with that HDFS ID.")
        return file_info
    except Exception as e:
...
def download_file(file_id, download_path=None):
    ...
    try:
        ...
        # Download from HDFS
        success = hdfs.download(file_id, download_path)
...
def update_file_metadata(file_id, new_metadata):
    """UPDATE: Update file metadata"""
    try:
        success = hdfs.update_metadata(file_id, new_metadata)
        if success:
...
def delete_all_files():
    """DELETE: Remove all files from HDFS"""
    try:
        count = hdfs.delete_all()
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            print("3.  Find file by HDFS ID (READ)")
            print("4.  Download file (READ)")
            print("5.  Update file metadata (UPDATE)")
            print("6.  Delete file by HDFS ID (DELETE)")
            print("7.  Delete ALL files ([WARNING] irreversible)")
            print("8.  Show total file count")
            print("9.  Parquet: create (overwrite)")
            print("10. Parquet: read")
            print("11. Parquet: update (overwrite)")
            print("12. Parquet: delete")
            print("0.  Exit")
            print("----------------")
            ...
            if choice == "1":
                ...
                file_id = input("Enter HDFS ID to delete: ")
                delete_file(file_id)
                ...
                # Example payload, user can customize
                print("Enter columns as comma-separated names (e.g., Name,Age)")
                ...
                print("[GOODBYE] Exiting HDFS... Goodbye!")
                break
